...
const JobApplicationForm = () => {
  ...
  // Only include fields that are used by the controller: experience and availability
  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        const res = await axios.get(`/api/job/${jobPostId}`);
        setJob(res.data);
      } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div className="bg-white overflow-hidden shadow rounded-lg">
          ...
          {/* Application Form */}
          {error && (
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 mx-4 rounded" role="alert">
              <span>{error}</span>
            </div>
          )}
